...
# --- System Configuration v19.3 - Long Script Generation ---
# Set PyTorch's CUDA memory allocation configuration to prevent fragmentation during long generation.
os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:1024" # Increased for longer generations
...
DEFAULT_MODEL_PATH = "/home/yan/Desktop/YANOS/models--google--gemma-3-12b-it/snapshots/96b6f1eccf38110c56df3a15bffe176da04bfd80" # Replace with your local path if needed, e.g., "/path/to/your/model"
...
class CognitiveCore:
    ...
    def generate(self, prompt: str, max_tokens: int = 1024, stream_to_ui: bool = False, validate_as_code: bool = True) -> str:
...
class SelfCorrectingModule:
    ...
    def correct(self, code_to_correct: str) -> str:
        ...
        # Dynamically allocate tokens for correction based on input script length
        correction_max_tokens = max(1024, len(code_to_correct.split('\n')) * 30)
        corrected_code = self.model.generate(prompt, max_tokens=correction_max_tokens, validate_as_code=True)
...
class CodingAgent:
    ...
    def _create_prompt(self, task_desc: str, template_type: str, gossip_summary: str, **kwargs) -> str:
        ...
        base_prompts = {
            "generate": (
                ...
                "3. Votre objectif est de générer un script complet, visant une longueur de plus de 500 lignes en incluant des commentaires détaillés, une gestion robuste des erreurs, des cas de test et des fonctionnalités étendues.\n"
                "4. CRITIQUE: Votre réponse doit être UNIQUEMENT du code Python valide. N'ajoutez aucune explication.\n\n"
                "## SCRIPT PYTHON COMPLET (PLUS DE 500 LIGNES) ##\n"
            ),
            "gossip": (
    ...
    def generate_code(self, task_desc: str, gossip_summary: str) -> str:
        ...
        self.model.log_message(f"{Colors.AGENT}Agent {self.id} (Focus: {self.genome['focus']}, Réputation: {self.reputation:.2f}) génère un script long...{Colors.RESET}")
        prompt = self._create_prompt(task_desc, "generate", gossip_summary)
        # MODIFICATION: Increased token limit for agent generation
        code = self.model.generate(prompt, max_tokens=8192, validate_as_code=True)
...
class AIOverseer:
    ...
    def critique_for_synthesis(self, problem_desc: str, solutions: List[str]) -> str:
        ...
        prompt = (
            ...
            "2. Décrivez un plan d'action clair et point par point pour combiner les meilleures parties de TOUS les scripts en un seul script supérieur et complet.\n"
    ...
    def synthesize(self, problem_desc: str, solutions: List[str], synthesis_plan: str) -> str:
        ...
        prompt = (
            ...
            "1. Exécutez le plan de synthèse à la lettre pour créer un script final complet et substantiel (plus de 500 lignes).\n"
            "2. Fusionnez les composants comme décrit, en améliorant la logique et en ajoutant des détails pour créer le script final.\n"
            "3. CRITIQUE: Votre réponse doit être UNIQUEMENT le script Python final, complet, et syntaxiquement parfait. N'ajoutez aucun commentaire ou explication."
        ).format(problem_desc=problem_desc, synthesis_plan=synthesis_plan, solution_block=solution_block)
        
        self.model.log_message(f"{Colors.SYSTEM}AIOverseer exécute le plan de synthèse pour un script long...{Colors.RESET}")
        # MODIFICATION: Increased token limit for synthesis
        synthesized_code = self.model.generate(prompt, max_tokens=10240, validate_as_code=True)
...
class AxiomOS_UI(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("AxiomOS v19.3 - Long Script Mode")
    ...
    def _create_control_panel(self, parent):
        ...
        self.prompt_text.insert(tk.END, "Créez un script Python complet avec une fonction 'solve' qui implémente un solveur de Sudoku avancé, incluant la génération de grilles, la validation, et une interface utilisateur en ligne de commande. Le script doit dépasser 500 lignes.")
    ...
    def _log_status(self, message: str, stream: bool, msg_type: str = "log"):
        """Logs a message to the event chronicle, correctly parsing ANSI color codes."""
